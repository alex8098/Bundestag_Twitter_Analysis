...
df = pd.read_csv("MDB Twitter.csv", encoding="latin")

# Attention we need a webdriver.exe and selenium extension for the next steps
driver = webdriver.Chrome("C:\\Program Files (x86)\\Google\\Chrome\\chrome\\chromedriver.exe")
sleep(3)
...
df_temp = pd.DataFrame(columns =["Twitteraccount", "Following", "Follower", "Ort"]).fillna(value="nan")
...
for acc in list_acc:
    if i<8:
        ...
        # save to dataframe
        df_temp["Following"][i] = follower[0]
